...
export function ProjectCard({ project, large = false }: { project: Project; large?: boolean }) {
  ...
  return (
    ...
    >
      {/* Hero image */}
      {hasImage && (
        <div className={`overflow-hidden ${large ? "aspect-video" : "aspect-[16/9]"}`}>
          <img
            src={project.image}
      ...
      {!hasImage && (
        <div
          className={`${large ? "aspect-video" : "aspect-[16/9]"} bg-gradient-to-br from-muted to-surface`}
        />
      )}

      {/* Content */}
      <div className={`flex flex-col ${large ? "p-8 md:p-12" : "p-7 md:p-9"}`}>
        ...
        <p className="mt-3 text-sm text-muted-foreground font-mono">{project.role}</p>
